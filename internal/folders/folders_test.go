...
func TestSetSaveLoadRoundTrip(t *testing.T) {
	...
	s, _ := Load(path)
	s.Set("/home/user/code/client", "client")
	s.Set("/home/user/code/foo", "default")
	if err := s.Save(); err != nil {
		t.Fatal(err)
	}

	s2, _ := Load(path)
	if v, ok := s2.Get("/home/user/code/client"); !ok || v != "client" {
		t.Fatalf("got %q ok=%v", v, ok)
	}
	if v, ok := s2.Get("/home/user/code/foo"); !ok || v != "default" {
		t.Fatalf("got %q ok=%v", v, ok)
	}
}
